...
# Assuming almost spherical Earth, 111697.78m per 1 degree, or 11.169778m per 31 pixels

points_of_interest = []
boat_data = [] # Boat Data: [longitude, latitude, rotation, winch_status, temperature, Ph]
# add package_number
package_size = 6
...
def boat_signal():
    ...
    if len(boat_data) == package_size:
        ...
        boat_data[0] = boat_data[0]/10000000
...
class MyGame(arcade.Window):
    ...
    def on_draw(self):
        ...
        if len(boat_data) == package_size:
            boat_x, boat_y = degrees_to_pixels(boat_data[0], boat_data[1])
            arcade.draw_circle_filled(boat_x, boat_y, 5, arcade.color.YELLOW_ROSE)
            arcade.draw_line(boat_x, boat_y, boat_x+sin(boat_data[2]*3.14/180)*10, boat_y+cos(boat_data[2]*3.14/180)*10,
            ...
            arcade.draw_text(f"{boat_data[0]}", 155, 615, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)

            arcade.draw_text(f"Latitude:", 20, 600, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
            arcade.draw_text(f" {boat_data[1]}", 150, 600, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)

            arcade.draw_text(f"Rotation:", 20, 585, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
            arcade.draw_text(f" {boat_data[2]}", 150, 585, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)

            arcade.draw_text(f"Winch Status:", 20, 560, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
            arcade.draw_text(f" {boat_data[3]}", 150, 560, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)

            arcade.draw_text(f"Temperature:", 20, 545, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
            arcade.draw_text(f" {boat_data[4]}", 150, 545, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)

            arcade.draw_text(f"Ph:", 20, 530, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
            arcade.draw_text(f" {boat_data[5]}", 150, 530, color=AIR_SUPERIORITY_BLUE, font_size=14, bold=True)
